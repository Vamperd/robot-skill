...
EVAL_SCENARIOS = [
    ...
    },
    {
        ...
        {
        "name": "hard_very_thin_corridor_layout",
    ...
    {
        "name": "ultimate_generalization_test",
        "start_pos": (60.0, 60.0),
        "obstacles": [
            # 1. 叹息之墙 (The Great Detour) - 逼迫小车横跨整个屏幕绕路
            (250, 0, 40, 480),   # 左侧巨墙：挡住去右边的路，只能从底部最下方缝隙过
            (450, 120, 40, 480), # 右侧巨墙：挡住去右下的路，只能从顶部最上方缝隙过
            
            # 2. 极限S型盲区 (The Squeeze) - 考验在长走廊中的绝对控制力
            (290, 280, 110, 40), # 夹在两堵巨墙中间的横向路障，右侧仅留 50 像素缝隙！
            
            # 3. 欺骗性 U 型陷阱 (The U-Trap) - 针对 Task B 的死锁考验
            (0, 400, 150, 40),   # 陷阱的上盖
            (150, 400, 40, 140), # 陷阱的右墙 (直面小车的来向，目标就在墙后，引诱它贴墙撞死)
            
            # 4. 精准车库 (The Dead End) - 针对 Task C 的死胡同
            (580, 360, 220, 40), # 车库上墙
            (580, 480, 220, 40), # 车库下墙
            (760, 400, 40, 80),  # 车库底墙
        ],
        "tasks": {
            "Task A": (680, 60),  # 位于地图右上角的空旷区
            "Task B": (80, 500),  # 被死死锁在左下角的 U 型陷阱深处
            "Task C": (680, 420), # 位于右下角极度狭窄的“横向车库”内部
        },
    },
]
