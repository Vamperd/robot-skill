...
for path in (NAV_DIR, SCHED_DIR):
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

from low_level_policy_adapter import LowLevelPolicyAdapter  # noqa: E402
from no_robot_avoid_nav_runner import NoRobotAvoidNavRunner  # noqa: E402
from scenario_generator import load_scenarios  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="运行实验版高层调度 + A* + 底层 PPO 联调。该路径只把墙壁视为障碍，机器人之间可重叠穿过。"
    )
...
def main() -> None:
    ...
    scheduler_policy = (
        NoRobotAvoidNavRunner.load_scheduler(args.scheduler_model, policy_type=args.scheduler_policy_type)
        if args.scheduler_model
        else args.scheduler_policy
    )
    runner = NoRobotAvoidNavRunner(
